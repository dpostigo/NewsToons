...
-(NSString *)localPath:(PhotoSize)photoSize
{
    if (photoSize < CC_SQUARE_75 || photoSize > CC_ORIGINAL) {
        [NSException raise:@"Invalid Photo Size" format:@"Unknown photo size"];
    }
...
{
	return [NSString stringWithFormat:@"%@/%@", [Cocoafish defaultCocoafish].cocoafishDir, self.objectId];
}
...
-(id)initWithJsonResponse:(NSDictionary *)jsonResponse
{
    ...
    self = [super init];
    if (self) {
        self.model = [jsonResponse objectForKey:@"_model"];
